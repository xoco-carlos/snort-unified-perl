...
sub readSnortUnifiedRecord() {
    ...
    $readsize = read(UFD, $buffer, $UF->{'RECORDSIZE'}, $readsize);
    while ( $readsize != $UF->{'RECORDSIZE'} ) {
	# reset EOF condition if it exists
	# This doesn't work on all platforms
	# seek(UFD,0,1);
        seek(UFD, $UF->{'FILEPOS'}+$readsize, 0);
        $readsize += read(UFD, $buffer, $UF->{'RECORDSIZE'}-$readsize, $readsize);
        $fsize = (stat(UFD))[7];
        $mtime = (stat(UFD))[9];
        
        debug("Read $readsize bytes so far.");
        debug("fpos is $fsize:$UF->{'FILEPOS'}.");
        debug("mtime is $mtime:$UF->{'FILEMTIME'}.");
 
        # if the file is unchanged track DEADREADS
        if ( ( $mtime eq $UF->{'FILEMTIME'} ) && 
             ( $fsize eq $UF->{'FILESIZE'} ) &&
             ( $fsize eq $UF->{'FILEPOS'} )) {
            $UF->{'DEADREADS'}++;
            debug("Checking DEADREADS of $UF->{'DEADREADS'}\n");
            if ( $UF->{'DEADREADS'} % 5 == 0 ) { 
	        # Bailing on too many dead reads
		debug("Bailing on too many DEADREADS\n");
		# reset fpos to the beginning of the record
		# NOTE: Could use whence of 2 with $recordsize to go from EOF but 
		# I'm speculating that a race exists whereby a write could
		# happen before we get here thus messing up alignment
		debug("Seeking to byte $UF->{'FILEPOS'}\n");
